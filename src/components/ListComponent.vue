      ...
      <table class="table table-hover table-bordered">
        ...
        <tbody class="text-center">
          <tr v-for="doctor in Doctors" :key="doctor._id">
            ...
            <td>
              <button @click.prevent="deleteDoctor(doctor._id)" class="btn btn-danger">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    deleteDoctor(id) {
      let indexOfArrayItem = this.Doctors.findIndex(
        doctor => doctor._id === id
      );
      let apiURL = `https://amonmr.herokuapp.com/api/v1/facilities-doctors/${id}`;
      alert(apiURL);
      if (window.confirm("Do you really want to delete?")) {
        ...
        axios
          ...
          .then(() => {
            this.Doctors.splice(indexOfArrayItem, 1);
          })
          .catch(error => {
            console.log(error);
            alert("Error! under maintenance ;)");
          });
      }
    },
    logout: function() {
